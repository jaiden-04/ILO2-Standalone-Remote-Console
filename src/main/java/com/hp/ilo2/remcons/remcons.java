...
public class remcons extends Applet implements ActionListener, ItemListener, TimerListener, Runnable, AppletStub {

    // ADDED AFTER DECOMPILATION
    ...
    private static final int INFINITE_TIMEOUT = Integer.MAX_VALUE - 7;
    private int session_timeout = SESSION_TIMEOUT_DEFAULT;

    private cim session;

    private Panel sessionWindow;
    ...
    private final String hp_mouse_warning = "The High Performance Mouse is supported natively on Microsoft Windows Server 2000 SP3 or later and Windows 2003 or later. " +
            "Linux users should enable the High-Performance Mouse option once the HP iLO2 High-Performance Mouse for Linux driver is installed.";
    ...
    public void init() {
        ...
        if (this.lt.showgui) {
            ...
            String[] locales = this.lt.getLocales();
            for (String locale : locales) {
                this.kbd_locale.add(locale);
            }
            if (str != null)
        ...
        this.toolBar = new Panel(new FlowLayout(1, 1, 7));

        this.toolBar.add(this.refresh);
        this.toolBar.add(this.term_svcs);
        this.toolBar.add(this.send_ctrl_alt_del);


        this.toolBar.add(this.alt_lock_label);
        this.toolBar.add(this.alt_lock);

        this.toolBar.add(this.hp_mouse_label);
        this.toolBar.add(this.hp_mouse);

        this.toolBar.add(this.local_cursor_label);
        this.toolBar.add(this.local_cursor);

        if (this.kbd_locale != null) {
            this.toolBar.add(this.kbd_locale_label);
            this.toolBar.add(this.kbd_locale);
        }
        ...
        this.sessionWindow = new Panel();


        this.sessionWindow.setLayout(new GridBagLayout());
        ...
        this.sessionWindow.add(this.toolBar, localObject);
        localObject.gridy = 1;
        this.sessionWindow.add(this.session, localObject);


        setLayout(new FlowLayout(0));
        add(this.sessionWindow);
    ...
    {
        this.login = parseLogin(getParameter("INFO0"));
        ...
        int i; if (str != null)
    {
        ...
        this.session_encryption_enabled = i == 1;
    } else {
        ...
        this.debug_msg = (str != null) && (str.length() > 0);
    ...
    private String parseLogin(String paramString) {
